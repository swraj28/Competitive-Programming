...
int binarySearch(int arr[], int n, int data) {   //BINARY SEARCH IS ALWAYS APPLICABLE ON SORTED ARRAY.
	//iterative approach (t.c-o(logn) and s.c-o(1)).
	int start = 0;
	int end = n - 1;
...
float square_root(int n, int precision) {
	...
	while (start <= end) //integral part of the square root
	{
	...
	for (int i = 0; i < precision; i++) //decimal part upto given precision
	{
		while (ans * ans <= n)
		{
			ans += increment;
...
void mergeSort(int arr[], int start, int end) {
	if (start == end) { // it means that the array is sorted and by traversing start pointer hit the end pointer
		return;
	}
